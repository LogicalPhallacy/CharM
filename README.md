...
### Mac - Currently probably broken
Run Charm. It is a little larger than it has to be because something in trimming breaks the database, but it should run. Building and Testing here is more limited annoyingly because of a lack of apple devices. If you are an apple user with dev experience feel free to help fix this.
### Android
...
On a character with unselected choices (like a new character or a character you levelled up), there should be some boxes at the top of the page indicating which choices are available. You can click them to select from the available options given the choice and your characters current state. If you don't like a choice you made, you can find all of them in the detail tab and remove it, some can also be reselected by clicking the little edit pencil right by the choice. Optional choices can be dismissed with the little x after them. Some choices may be selectable multiple times (the choice box will have a number after it if it is). You can click the "show optional choices" button on the character overview panel to restore the choices box if its away.
...
- Allowing/Disabling options by source book
...
- The print UI could probably be better or support some sort of templating engine.
- Mac OS still doesn't really work
